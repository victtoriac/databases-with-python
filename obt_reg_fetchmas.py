...
if __name__=='__main__':
    try:
        ...
        #query="SELECT id,username email FROM users ORDER BY id DESC" #ordeno de mayor a menor
        #query="SELECT * FROM users LIMIT 3" limito cant registros q quiero 
        query="SELECT * FROM users"
        rows=cursor.execute(query)
        #print(rows) retorna 9 q es la cant registros q tengo 
        for user in cursor.fetchmany(3):  #fetchall me da tupla de los datos de cada user 
            print(user)
